...
**Image contents.** The Alphabyte wordmark + icon lockup in the upper-left corner. Centred-left, the headline **"AI consulting that compounds."** set big in Geist with brand-blue accent on "compounds." A subtle byte-cube glyph in the lower-right corner at low opacity. Off-white canvas background. Editorial composition with generous whitespace. No photography, no people, no gradients.
...
- **File:** `public/og/our-work-circular-economy-platform.png` · 1200×630 · PNG

**Page contents.** A case study about an executive intelligence + auction analytics platform Alphabyte built for an anonymised circular economy startup. Includes a Market Supply Index (MSI) and bid-band model that gave the exec team real-time visibility into supply curves and pricing decisions. Live in production.
...
- **File:** `public/og/our-work-housing-services-corp.png` · 1200×630 · PNG
...
- **File:** `public/blog/why-ai-pilots-stall-hero.webp` · 1600×900 · WebP
...
- **File:** `public/blog/citizen-developer-enablement-playbook-hero.webp` · 1600×900 · WebP
...
**Image contents.** A nested-rectangles framework on off-white canvas: an outer rectangle (governance perimeter) containing four inner rectangles (the four control domains discussed in the post). All in border-default grey outlines, with the rectangle for "data governance" filled in alphabyte-blue. **No legal / gavel imagery. No flags. Pure structural diagram.**
...
**Image contents.** A scattering of small grey dots across off-white canvas (the shadow activity), with three of them on the right connected by thin alphabyte-blue lines into a small clustered shape (the governed cluster). Editorial spacing, no labels, no decorative styling. **No "spy / hooded figure / hacker" clichés. No red. No green.**
...
**Image contents.** A variation on the comparison-frame, deliberately distinguished from #10. Off-white canvas with two columns: left has a wider, fuller set of horizontal bars in border-default grey (representing "broad Office surface coverage"), right has fewer but blue-filled bars (representing "deeper Claude integration"). Asymmetric weights. **NO Microsoft / Copilot logo or wordmark. Visual must read differently from #10.**
...
**Image contents.** A 4×2 grid of eight identical thin-bordered tiles on off-white canvas. Two tiles in the top row are filled with alphabyte-blue (the "first two workflows you should run"); the other six are blank. Hairline border-default grey on every tile. **No spreadsheet imagery. No $$$ icons. No calculator clichés.**
...
**Image contents.** A simple topology diagram on off-white canvas: a Claude node (filled blue circle) connected by thin lines to three system nodes (outlined squares in border-default grey). The MCP server itself sits as a small filled square between Claude and the three systems. Optional Geist Mono mini-labels in muted grey ("system A / B / C"), or no labels at all. **No real product logos (no Salesforce / Slack / etc.). No code screenshots.**
...
- **File:** `public/blog/private-llm-vs-claude-enterprise-hero.webp` · 1600×900 · WebP

**Page contents.** *"Private LLM vs Claude Enterprise: when you need on-premise."* Argues that most mid-market companies do not need an on-premise LLM, and offers a practical decision framework for data sovereignty, compliance, and when Claude Enterprise is enough. Tags: Data & Infrastructure, Claude. 6 min read.
...
- **File:** `public/our-work/housing-services-corp-roadmap-concept.webp` · 1200×800 · WebP
- *(Note: requires the same `CaseStudyPage` component update.)*

**Page contents.** Same case study as #4 (housing services / AI enablement roadmap). The page is currently a "Coming Soon" stub. A single supporting visual converts it from placeholder to substantive overnight, even before fuller copy is written.
...
# Brand guardrails (apply to every image above)

- **Colours, only these:** `#00ABF0` blue · `#00FFA6` green · `#171717` ink · `#FAFAFA` canvas · `#EBEBEB` grey · `#E5E5E5` border · `#737373` muted · `#FFFFFF` white
- **Type, only these:** Geist (display + body) · Geist Mono (small labels and IDs)
- **Forbidden across the set:** purple/violet · deep navy `#0D1117` · emoji icons · SVG-drawn faces or scenes · stock photography · neon/cyberpunk · "two professionals shaking hands" · gradient meshes · client logos · real product UIs · real chapter / case / numerical data · government insignia · code screenshots · server-room photography
- **Voice:** plain, contrarian, anti-fluff. British English in any baked-in copy (`programme`, `behaviour`).
